...
def test_init_ts(test_ts, test_pd_df):
    df = ScmDataFrame(test_ts, columns={
        'index': [2005, 2010],
        'model': ['a_iam'],
...
def test_append(test_scm_df):
    ...
    pd.testing.assert_series_equal(df.meta['col2'], pd.Series([np.nan, np.nan, np.nan, 'b']), check_names=False)
...
def test_set_meta_no_name(test_scm_df):
    idx = pd.MultiIndex(levels=[['a_scenario'], ['a_iam'], ['World']],
                        labels=[[0], [0], [0]],
...
def test_set_meta_as_str(test_scm_df):
    ...
    exp = pd.Series(data=['testing', 'testing', 'testing'], index=test_scm_df.meta.index, name='meta_str')
...
def test_rename_variable(test_scm_df):
    mapping = {'variable': {'Primary Energy': 'Primary Energy|Total', 'Primary Energy|Coal': 'Primary Energy|Fossil'}}

    obs = test_scm_df.rename(mapping)

    exp = pd.Series(['Primary Energy|Total', 'Primary Energy|Fossil', 'Primary Energy|Total'])
    pd.testing.assert_series_equal(obs['variable'], exp, check_index_type=False, check_names=False)
